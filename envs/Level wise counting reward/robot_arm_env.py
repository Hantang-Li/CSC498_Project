...
class RobotArmEnvV1(RobotArmEnvV0):
    def __init__(self):
        ...
        # circle reward
        self.reward_left = 10
        self.reward_deduced = 0
        self.previous_distance = float('inf')
        self.total_arm_length = sum(self.links) # fixed
        self.distance_percentage = [0.6, 0.3, 0.2, 0.15, 0.1, 0.05] # fixed
        self.distance_reward = [1, 1, 1, 2, 2, 3] # fixed
        self.reward_obtained = [False, False, False, False, False, False] # fixed
        self.reward_num = 6 # fixed
    ...
    def step(self, action):
        ...
        # discrete reward
        def reduce_reward_left():
            reward = 0
            done = False
            # Check if the robot arm is initialized at a point inside the reward cycle
            if self.previous_distance == float('inf'):
                distance_percentage = distance_error/self.total_arm_length
                for i in range(self.reward_num):
                    if distance_percentage <= self.distance_percentage[i]:
                        self.reward_obtained[i] = True
                        reward = reward + self.distance_reward[i]
            else:
                # the agent is able to obtain reward only if there's reward left
                if self.reward_left > 0:
                    distance_percentage = distance_error/self.total_arm_length
                    for i in range(self.reward_num):
                        # If the reward at the i th donut shaped area has not obtained yet
                        if not self.reward_obtained[i]:
                            if distance_percentage <= self.distance_percentage[i]:
                                self.reward_obtained[i] = True
                                reward = reward + self.distance_reward[i]
            
            # reduce total reward left
            self.reward_left = self.reward_left - reward

            # check if robot arm moves further
            if  self.previous_distance < distance_error:
                reward = reward - 1
                self.reward_deduced = self.reward_deduced + 1
            ...
            # done if robot move further such that the reward deduced due to move further becoms greater or equal than the reward total gained plus one
            # by doing this, we encouraged the agent to move closer to the goal, and restrict the time limit
            # the distance between tip and ball must decrease otherwise reward will be minused and until the reward minused is greater than 1+reward gained
            # here we also constrained the reward to be within -1 to 1, due to numerical reasons.
            if self.reward_deduced >= 20-self.reward_left:
                done = True
            
            return reward, done
        
        reward, done = reduce_reward_left()
        # test
        #print(reward, done, distance_error, self.total_arm_length, self.reward_left, self.reward_deduced)
